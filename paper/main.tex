...
The representation probes provide the strongest result. CORAL nearly eliminates covariance discrepancy ($>$99.6\% reduction; Appendix~\ref{sec:alignment}) while the shortcut remains perfectly decodable ($x_1$ AUC~$= 1.000$). An independent domain probe shows DANN's representations remain perfectly domain-separable (probe AUC~$= 1.000$; Appendix~\ref{sec:alignment}), while DANN erodes the causal signal ($x_0$ AUC: $0.829 \to 0.651$) without eliminating shortcut information ($x_1$ AUC~$= 0.993$). Under prior-penalized optimization preventing latent extrapolation, the CORAL oracle reliably degrades design ($p_{\text{Holm}} = 0.008$, 9/10 seeds negative), confirming this failure persists near the VAE manifold. The core failure is not merely poor domain adaptation, but rather a fundamental inability to handle conditional shift: enforcing invariance across domains whose underlying causal mechanism has changed~\cite{zhao2019learning}.
