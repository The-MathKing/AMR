...
\begin{abstract}
Predictive oracles are central to \textit{in-silico} biomolecular design, but optimizing an oracle is only useful when its learned associations remain valid for the biological target of interest. Using a controlled semi-synthetic stress test parameterized by the resistance prevalence of 3,975 \textit{Mycobacterium tuberculosis} records---where genomic features, peptide latent factors, and drug-target efficacy are all simulated---we investigate how population-specific shortcuts in AMR prediction propagate into antimicrobial-peptide latent-space optimization. CORAL reduces source-target covariance discrepancy by more than 99.6\% while leaving the simulated population-associated shortcut perfectly linearly decodable; DANN erodes transferable information while both shortcut and domain identity remain recoverable. Under matched prior-regularized optimization, CORAL improves efficacy relative to a source-only oracle when source and target share an efficacy mechanism, but produces lower efficacy when the target mechanism changes (difference-of-differences permutation test, $p = 0.0039$). We interpret these experiments as a controlled oracle-reliability stress test rather than realistic peptide-engineering validation.
\end{abstract}
