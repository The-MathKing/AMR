...
Beyond clinical diagnostics, generative antimicrobial design pipelines commonly rely on learned activity predictors or screening oracles to rank novel antimicrobial peptide (AMP) candidates \cite{das2021accelerated, szymczak2023hydramp}. When a predictor becomes an optimization objective, shortcut learning is amplified: its errors determine not only what is predicted, but where candidate search is directed. If those oracles inherit population-specific resistance shortcuts, this is no longer just an accuracy issue---it directly corrupts the generative design loop, causing the pipeline to prioritize ineffective molecules. Indeed, recent work has independently recognized that optimizing learned molecular-property predictors can produce high predicted scores but poor true outcomes \cite{gendreau2023assays, yoshizawa2025dyramo}.
...
In this paper, we construct a controlled semi-synthetic stress test parameterized by clinical resistance prevalences (3,975 isolate phenotypes) that models the separation of causal features from spurious source-domain shortcut features. We first evaluate whether alignment-based deconfounding methods, including DANN and Deep CORAL \cite{sun2016deep}, can correct the diagnostic failure. Finding that they cannot, we extend the analysis to a generative design loop, demonstrating how this lingering confounding can misdirect downstream candidate optimization.
